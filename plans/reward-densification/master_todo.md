...
- **`mark-to-market-smoothing`** — if probe shows high
  ...
  refinement only.
- **`arb-curriculum`** (scaffolded 2026-04-19 at
  `plans/arb-curriculum/`) — if the 2026-04-19 gene-sweep
  (`reward-densification-gene-sweep`) confirms the structural
  diagnosis (the policy finds "arb less" before "arb better"
  because the aggregate gradient at init points against all
  arbing), this is the next planned work. Seven sessions:
  offline oracle scan, matured-arb shaped bonus, naked-loss
  annealing, BC pretrain + target-entropy controller handshake,
  curriculum day ordering (arb-rich days first), registry
  reset + probe plan, validation launch. See
  `plans/arb-curriculum/purpose.md` for the full diagnosis
  and design sketch. Inherits the "per-agent BC, never
  shared" hard-constraint from `plans/arb-improvements/`
  (that folder's BC scoping was never built; arb-curriculum
  picks up the parts of its design that still apply).
