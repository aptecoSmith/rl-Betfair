...
exec >> registry/_roundP_wrapper.log 2>&1
echo "[$(date -Iseconds)] roundP started"
...
run_cell () { local cell="$1"; shift; local ts; ts=$(date +%s); local o="registry/_roundP_${cell}_${ts}"; echo "[$(date -Iseconds)] starting ${cell}"; python -m training_v2.cohort.runner "${BASE[@]}" --seed "${SD:-42}" --output-dir "$o" "$@" > "$o.log" 2>&1; echo "[$(date -Iseconds)] ${cell} rc=$?"; }
SD=42 run_cell P_seed42 --reward-overrides force_close_before_off_seconds=120
SD=43 run_cell P_seed43 --reward-overrides force_close_before_off_seconds=120
SD=44 run_cell P_seed44 --reward-overrides force_close_before_off_seconds=120
SD=45 run_cell P_seed45 --reward-overrides force_close_before_off_seconds=120
SD=46 run_cell P_seed46 --reward-overrides force_close_before_off_seconds=120
SD=47 run_cell P_seed47 --reward-overrides force_close_before_off_seconds=120
echo "[$(date -Iseconds)] roundP fan-out complete"
